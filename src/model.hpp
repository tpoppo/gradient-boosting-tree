...
namespace ogbt {
using TreeGenerator = std::function<Tree(const DatasetTest &, const std::vector<float> &)>;
...
public:
  Model(TreeGenerator t_tree_generator,
    ...
    const float t_learning_rate = 0.1) noexcept
  ...
  void fit(const Dataset &data) {
    ...
    std::vector<float> y_pred(data.get_y().size());
  ...
  std::vector<float> predict(DatasetTest t_data) const noexcept {
    std::vector<float> data(t_data[0].size());
    for (const auto &tree : ensemble_trees) {
      std::vector<float> target_prediction = tree.predict(t_data);
