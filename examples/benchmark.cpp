...
std::pair<ogbt::Dataset, ogbt::Dataset> get_dataset() {
  ...
  return std::make_pair(ogbt::get_dummy_data(100000, 5, rng), ogbt::get_dummy_data(50000, 5, rng));
}
...
void evaluate_algo(std::string name,
  ...
  std::cout << "[" << name << "] score: " << score_val << " (" << score << ") time(ms): " << since(start).count() << std::endl;
}
...
int main() {
  ...
  evaluate_algo("genetic_algo", 100, dataset, dataset_validation, [](const ogbt::DatasetTest &x, const std::vector<double> &y) {
  ...
  evaluate_algo("greedy_mse_splitting", 100, dataset, dataset_validation, [](const ogbt::DatasetTest &x, const std::vector<double> &y) {
    const unsigned tree_depth = 6;
    const double subsample_a = 0.5;
    const double subsample_b = 0.0;
